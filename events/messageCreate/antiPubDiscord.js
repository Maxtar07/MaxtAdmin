...
module.exports = async (bot, message) => {
  ...
  if ((!(message.channel.id === `726428195187851321` ||
    ...
    message.member.roles.cache.find(m => m.id === `844991387668709416`))) && (message.content.toLocaleLowerCase().includes(`discord.gg`))) {
    await message.delete({ timeout: 0 })
    const targetUserId = message.member.user.id
    const reason = `Attention, les pubs pour discord c'est pas ici !`;
