...
module.exports = async (bot, message) => {
  ...
  if ((!(message.channel.id === `726428195636903971` ||
    message.member.roles.cache.find(m => m.id === `844991387668709416`))) && (message.content.toLocaleLowerCase().includes(`youtube.com/channel`))) {
    await message.delete({ timeout: 0 })
    const targetUserId = message.member.user.id
    const reason = `Attention, les pubs pour YouTube c'est pas ici !`;
