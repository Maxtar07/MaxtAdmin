...
module.exports = (localCommand, botCommand) => {
  const differences = {};
  if (localCommand.name !== botCommand.name) {
    differences.name = true;
  }
  if (localCommand.description !== botCommand.description) {
    differences.description = true;
  }
  const localOptions = localCommand.options || [];
  const botOptions = botCommand.options || [];
